...
begin
	...
	#we do have the ec numbers, ratios we need are listed under [Met]/Km column


end

# ╔═╡ c682807a-06cd-49db-a09b-29719b623ee6
begin

	
	# setup flux bounds array -
	met_bounds_array = zeros(ℛ,2)
	met_bounds_array[:,2] .= 100 # default value is 100 for flux units: μmol/gDW-s
	met_bounds_array[1,2] = 203.0*E[1]*11.9*96.7/(97.7*12.9-1)
	met_bounds_array[2,2] = 34.5*E[2]*0.0915*0.0852/(1.0915*1.0852-1)
	met_bounds_array[3,2] = 249.0*E[3]*0.165*1.39/(1.165*2.39-1)
	met_bounds_array[4,2] = 88.1*E[4]*2.81*0.0119/(3.81*1.0119-1)
	met_bounds_array[5,2] = 13.7*E[5]*73.1/73.1
	met_bounds_array[6,2] = 13.7*E[6]*73.1/73.1

	# compute the flux -
	result_case_2 = lib.flux(S,met_bounds_array,species_bounds_array,c_vector);

	# show -
	nothing
end

# ╔═╡ 3e160ea9-ed7c-491a-9af0-efe817ae4155
# check:
with_terminal() do
...
# ╔═╡ 114814bb-e8c6-4a18-abf9-9d72ed41b85a
let

	# get flux values from the result -
	calculated_flux_array = result_case_2.calculated_flux_array
